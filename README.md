...
> **Status:** a Rust service runs locally with strict configuration, structured process telemetry, operator health routes (`/live`, `/ready`, `/metrics`, `/version`), stable repository identity and star/list synchronization, encrypted replacement-PAT storage, audited repository modes and provider mutations, and desired-backup-policy publication. A separate loopback domain listener serves Edge-authenticated repository preview and confirmed `metadata`/`track`/`star` actions with durable replay and component-level partial results. A third, private-network listener exposes only a separately service-authenticated, owner-scoped immutable README resolver for Knowledge; it verifies the retained publication and BlobRef before returning bounded bytes. OAuth, an internet-facing API, and live fleet-bus handlers remain planned.
